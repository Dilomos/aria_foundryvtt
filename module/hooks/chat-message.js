...
Hooks.on("chatMessage", (html, content, msg) => {
    ...
    const actor = game.aria.macros.getSpeakersActor();

    const validCommands = ["for", "str", "dex", "end", "int", "sag", "cha", "atc", "melee", "atd", "ranged", "atm", "magic"];

    if(command && validCommands.includes(command)) {
        game.aria.macros.rollStatMacro(actor, command, null);
        return false;
    }
    else if(command && command === "skill") {
        if(arg1 && validCommands.includes(arg1)) {
            game.aria.macros.rollStatMacro(actor, arg1, null);
        } else {
